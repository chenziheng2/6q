...
static int yt8521_phy_fixup(struct phy_device *dev)
{
	...
	phy_write(dev, 4, 0x11e1);
	phy_write(dev, 0x0, 0x100);
 
	phy_write(dev, 0x1e, 0xa00e);
	phy_write(dev, 0x1f, 0x40);

	phy_write(dev, 0x1e, 0xa00d);
	phy_write(dev, 0x1f, 0x660);
 
	phy_write(dev, 0x1e, 0xa012);
	phy_write(dev, 0x1f, 0x78);

	phy_write(dev, 0x1e, 0xa003);
	phy_write(dev, 0x1f, 0xfd);
...
static void __init imx6q_enet_phy_init(void)
{
	if (IS_BUILTIN(CONFIG_PHYLIB)) {
		...
		phy_register_fixup_for_uid(PHY_ID_AR8035, 0xffffffef,
				ar8035_phy_fixup);
		phy_register_fixup_for_uid(PHY_ID_YT8521, 0x0000011a,
				yt8521_phy_fixup);
